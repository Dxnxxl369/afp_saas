...
class RolesSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # Check for uniqueness of nombre within the same empresa
        query = Roles.objects.filter(empresa=empresa, nombre__iexact=nombre)
        ...
        if query.exists():
            raise serializers.ValidationError({
                "nombre": f"Ya existe un rol con el nombre '{nombre}' en la empresa '{empresa.nombre}'."
            })
        return data
...
class ActivoFijoSerializer(serializers.ModelSerializer): # <-- [EDITADO]
    ...
    class Meta:
        model = ActivoFijo
        fields = '__all__' # Incluye 'foto_activo' y 'solicitud_origen'
        read_only_fields = ('solicitud_origen',) # Hacemos que no se pueda asignar directamente
...
class SolicitudActivoSerializer(serializers.ModelSerializer):
    empresa = serializers.HiddenField(default=CurrentUserEmpresaDefault())
    solicitante = EmpleadoSimpleSerializer(read_only=True)
    departamento = DepartamentoSerializer(read_only=True)
    aprobado_por = EmpleadoSimpleSerializer(read_only=True)
    
    departamento_id = serializers.PrimaryKeyRelatedField(
        queryset=Departamento.objects.all(), source='departamento', write_only=True
    )
    
    # Campo para mostrar el estado legible para humanos
    estado_display = serializers.CharField(source='get_estado_display', read_only=True)

    class Meta:
        model = SolicitudActivo
        fields = [
            'id', 'empresa', 'solicitante', 'departamento', 'producto_solicitado',
            'cantidad', 'motivo', 'estado', 'estado_display', 'fecha_solicitud',
            'aprobado_por', 'fecha_aprobacion', 'notas_aprobacion',
            # Write-only fields
            'departamento_id',
        ]
        read_only_fields = (
            'estado', 'solicitante', 'aprobado_por', 
            'fecha_aprobacion', 'empresa'
        )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Filtrar el queryset de departamento_id por la empresa del usuario
        request = self.context.get('request')
        if request and hasattr(request.user, 'empleado'):
            empresa = request.user.empleado.empresa
            self.fields['departamento_id'].queryset = Departamento.objects.filter(empresa=empresa)
