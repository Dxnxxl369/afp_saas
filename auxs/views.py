...
class SolicitudActivoViewSet(BaseTenantViewSet):
    queryset = SolicitudActivo.objects.all().select_related(
        'solicitante__usuario', 'departamento', 'aprobado_por__usuario'
    ).order_by('-fecha_solicitud')
    serializer_class = SolicitudActivoSerializer
    # El permiso para crear se valida en perform_create, para ver en get_queryset
    # y para aprobar/rechazar en las acciones.

    def get_queryset(self):
        qs = super().get_queryset() # Filtra por empresa
        user = self.request.user

        # Si no es admin/staff, filtramos más
        if not user.is_staff and hasattr(user, 'empleado'):
            # Si el usuario NO tiene permiso para gestionar, solo ve las suyas
            if not check_permission(self.request, self, 'manage_solicitudactivo'):
                qs = qs.filter(solicitante=user.empleado)
        return qs

    def perform_create(self, serializer):
        # Asignar el solicitante automáticamente al crear
        serializer.save(solicitante=self.request.user.empleado)

    @action(detail=True, methods=['post'], url_path='aprobar')
    def aprobar(self, request, pk=None):
        if not check_permission(request, self, 'manage_solicitudactivo'):
            self.permission_denied(request, message='No tienes permiso para aprobar solicitudes.')

        solicitud = self.get_object()
        if solicitud.estado != 'PENDIENTE':
            return Response({'detail': 'La solicitud ya ha sido procesada.'}, status=status.HTTP_400_BAD_REQUEST)

        solicitud.estado = 'APROBADO'
        solicitud.aprobado_por = request.user.empleado
        solicitud.fecha_aprobacion = timezone.now()
        solicitud.notas_aprobacion = request.data.get('notas', '')
        solicitud.save()

        # Notificar al solicitante
        Notificacion.objects.create(
            destinatario=solicitud.solicitante.usuario,
            mensaje=f"Tu solicitud para '{solicitud.producto_solicitado}' ha sido aprobada.",
            tipo='INFO',
            url_destino=f'/app/solicitudes/{solicitud.id}'
        )
    ...
    def rechazar(self, request, pk=None):
        if not check_permission(request, self, 'manage_solicitudactivo'):
            self.permission_denied(request, message='No tienes permiso para rechazar solicitudes.')
        ...
        solicitud.aprobado_por = request.user.empleado
        solicitud.fecha_aprobacion = timezone.now()
        solicitud.notas_aprobacion = request.data.get('notas', 'Rechazado sin notas.')
        solicitud.save()

        # Notificar al solicitante
        Notificacion.objects.create(
            destinatario=solicitud.solicitante.usuario,
            mensaje=f"Tu solicitud para '{solicitud.producto_solicitado}' ha sido rechazada.",
            tipo='ADVERTENCIA',
            url_destino=f'/app/solicitudes/{solicitud.id}'
        )
        return Response(self.get_serializer(solicitud).data)

class ActivoQRView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, activo_id):
        try:
            # Aseguramos que el activo pertenezca a la empresa del usuario
            empleado = request.user.empleado
            activo = ActivoFijo.objects.get(id=activo_id, empresa=empleado.empresa)
            
            # La URL que contendrá el QR. Apunta a tu frontend.
            # TODO: Ajusta la URL base a tu dominio de producción
            qr_content = f"http://localhost:5173/app/activos/{activo.id}"
            
            qr_image = make_qr(qr_content)
            buffer = io.BytesIO()
